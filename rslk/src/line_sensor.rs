...
impl LineSensorBusy {
    ...
    pub fn peek_result(&self) -> u8{
        unsafe {
            let res = ((self.out7.is_high().ok().unwrap_unchecked() as u8) << 7u8)
                    | ((self.out6.is_high().ok().unwrap_unchecked() as u8) << 6u8)
                    | ((self.out5.is_high().ok().unwrap_unchecked() as u8) << 5u8)
                    | ((self.out4.is_high().ok().unwrap_unchecked() as u8) << 4u8)
                    | ((self.out3.is_high().ok().unwrap_unchecked() as u8) << 3u8)
                    | ((self.out2.is_high().ok().unwrap_unchecked() as u8) << 2u8)
                    | ((self.out1.is_high().ok().unwrap_unchecked() as u8) << 1u8)
                    | (self.out0.is_high().ok().unwrap_unchecked() as u8);
            res
        }
    }
